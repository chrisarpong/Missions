...
import ServiceModal from '@/components/crm/ServiceModal';
...
export default function CRMServices() {
  ...
  const load = () => base44.entities.ServiceInfo.list().then(d => { setServices(d); setLoading(false); });
  useEffect(() => { load(); }, []);
  ...
  const handleDelete = async (id) => {
    if (confirm('Delete this service?')) {
      await base44.entities.ServiceInfo.delete(id);
      load();
    }
  };
  ...
  return (
    <div>
      ...
      <p className="text-xs text-gray-400 mb-3">{filtered.length} service{filtered.length !== 1 ? 's' : ''}</p>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50">
              ...
              <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide hidden sm:table-cell">Processing Time</th>
            ...
            {filtered.map(s => (
              <tr key={s.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-3 text-xs">
                  <span className={`px-2 py-0.5 rounded-full font-medium ${s.category === 'Consular' ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700'}`}>
                    {s.category}
                  </span>
                </td>
                <td className="px-4 py-3 text-xs text-gray-500 hidden sm:table-cell">{s.processing_time || '—'}</td>
      ...
      {modalOpen && <ServiceModal service={editing} onSave={handleSave} onClose={() => { setModalOpen(false); setEditing(null); }} />}
    </div>
  );
}
